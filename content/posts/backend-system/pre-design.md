...
對於後端系統設計主要包括水平擴張、快取、非同步、避免單點故障、監控等等，甚至是「節約架構 (The Frugal Architect)」
...
這種類型最常見的問題是：「**可能會在極短時間湧入極大量的流量**」，且社群媒體系統都偏向 read-heavy system，經常會使用 cache 和 pre-build 優化資訊流檢索，專注討論如何處理尖峰時段的吞吐量問題。
...
{{< alert warning >}}
在 OLAP 這個背景下，「資料倉儲(Data Warehouse)」與「資料湖 (Data Lake)」的概念被提出，透過 ETL 的過程 (Extract-Transform-Load) 的方式，存到 Data Warehouse 中，而資料科學家與分析師，可以直接對 Data Warehouse 做查詢而不對 Sales DB 或 Inventory DB 做查詢，這樣就不會影響到後端 Server。
{{< /alert >}}

{{< alert success >}}
- 資料倉儲 Data Warehouse: 使用 ETL 過程會先做資料清洗、格式統一化等，這預先定義好資料結構再載入，雖然能讓查詢時能用輕易用最佳化的 SQL 語句來查，但同時也降低了彈性
