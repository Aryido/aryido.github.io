...
<!--BODY-->
> Heap 的定義是「 父節點和子節點之間滿足固定大小關係的 Tree 」，當所有父節點都**大於**其子節點時，就稱其為「Max Heap」; 當所有父節點都**小於**其子節點時，就稱其為「Min Heap」，[附上一個 Heap 圖參考連結](https://www.geeksforgeeks.org/dsa/heap-data-structure/)。 Heap 本身定義上**只關心節點之間的順序規則**，所以可以有很多種類例如： d-ary heap、binomial heap、Fibonacci heap 等等，但很多時候教材講的 Heap，其實默認會是 complete binary heap ，也就是除了滿足 heap property 還滿足了：
...
{{< alert warning >}}
「資料結構上的 Heap」 和討論「記憶體所提到的 Heap」 ，這兩個完全不一樣
...
由於我們經常默認 Heap 會是 **complete binary**，故很適合用 array 表示，因為結構很緊密，中間不會亂空。另外這個 Heap 的 root 節點代表整個 Heap 裡的最大值（max heap）或者最小值（min heap），例如下圖：
```text
        100
       /   \
     19     36
...
....
```
這個為一個 max heap，另外也可以用一個 list 來表示，由上到下，由左至右的排列在 list 中：

{{< image classes="fancybox fig-100" src="/images/data-structure/heap-list.jpg" >}}

從上圖中可以發現，**Heap 不是完全排序過的資料結構**，只有部份有序 ; 同一個 level 中，其實 node 之間並沒有什麼特別的關係。
...
- 如果 list 中一開始有資料，可以用 `heapify` 來初始化，會在線性時間內將 list 轉為 heap，且過程不會申請額外記憶體
    {{< alert info >}}
heapq.heapify(nums) 會直接原地修改 nums，回傳值是 None
{{< /alert >}}
- 插入與刪除的時間複雜度都是 `O(logN)`
- 你只是要看 heapq 的 top 元素，不拿出來，直接看 index 0 : `heap[0]`

- `heapq.heapreplace(heap, item)`
    `heapreplace` 比較特別，它會從 heap 取出並回傳最小的元素，接著將新的 item 放進 heap，所以 heap 的 size 不會改變，且如果 heap 是空的會產生 IndexError 錯誤。

    {{< alert info >}}
`heapreplace()` 一次完成的操作會比呼叫 `heappop()` 之後呼叫 `heappush()` 更有效率{{< /alert >}}

    {{< alert danger >}}
heapreplace 的回傳值可能會大於要加入的 item ，這點要特別注意
{{< /alert >}}
...
{{< alert success >}}
另外只要看到是 Top K 問題，基本上都可以嘗試使用 heap 來解題，這算是常見的套路
{{< /alert >}}
...
故所有顧客飲料全部做完時間是 10 mins。


{{< alert info >}}
可以注意一下 `heapq.heapify()` 操作， 還蠻方便的
{{< /alert >}}
