...
透過 init 建立專案之後，會**自動使用 Git 來管理**。且 uv 工具會產生了一些預設檔，主要有兩個重要的 file 來記錄資訊：

- **pyproject.toml** ：專案的主要依賴，包括專案名稱、版本、支援的 Python 版本等訊息
- **uv.lock** ：確保在不同環境下安裝的一致性
  {{< alert warning >}}
  `uv.lock` 這個檔案就是專案依賴關係的完整快照，它能確保專案在不同機器上的運作環境保持一致，詳細記錄了每個套件以及它依賴的套件的情況，由 uv 自動管理，不要自己去手動編輯
  {{< /alert >}}
...
```
這時候 Ruff 就是脫離目前的專案獨立運作的，可以使用 `which` 來查看到它的路徑並不是當前的虛擬環境
